...
type gameMatchR struct {
	Game *GameTitle `boil:"Game" json:"Game" toml:"Game" yaml:"Game"`
...
func (r *gameMatchR) GetGame() *GameTitle {
	if r == nil {
		return nil
	}
	return r.Game
...
func (r *gameMatchR) GetUser() *User {
	if r == nil {
		return nil
	}
	return r.User
...
func (q gameMatchQuery) One(ctx context.Context, exec boil.ContextExecutor) (*GameMatch, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "models_psql: failed to execute a one query for game_matches")
	}
...
func (q gameMatchQuery) All(ctx context.Context, exec boil.ContextExecutor) (GameMatchSlice, error) {
	...
	err := q.Bind(ctx, exec, &o)
	if err != nil {
		return nil, errors.Wrap(err, "models_psql: failed to assign all query results to GameMatch slice")
	}
...
func (q gameMatchQuery) Count(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "models_psql: failed to count game_matches rows")
	}
...
func (q gameMatchQuery) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	...
	err := q.Query.QueryRowContext(ctx, exec).Scan(&count)
	if err != nil {
		return false, errors.Wrap(err, "models_psql: failed to check if game_matches exists")
	}
...
func (o *GameMatch) Game(mods ...qm.QueryMod) gameTitleQuery {
	...
	return GameTitles(queryMods...)
}

// User pointed to by the foreign key.
func (o *GameMatch) User(mods ...qm.QueryMod) userQuery {
	queryMods := []qm.QueryMod{
...
func (gameMatchL) LoadGame(ctx context.Context, e boil.ContextExecutor, singular bool, maybeGameMatch interface{}, mods queries.Applicator) error {
	var slice []*GameMatch
	var object *GameMatch

	if singular {
		var ok bool
		object, ok = maybeGameMatch.(*GameMatch)
		if !ok {
			object = new(GameMatch)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeGameMatch)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeGameMatch))
			}
		}
	} else {
		s, ok := maybeGameMatch.(*[]*GameMatch)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeGameMatch)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeGameMatch))
			}
		}
	}
	...
	query := NewQuery(
		qm.From(`game_titles`),
		qm.WhereIn(`game_titles.id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load GameTitle")
	}

	var resultSlice []*GameTitle
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice GameTitle")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results of eager load for game_titles")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for game_titles")
	}
	...
	if singular {
		...
		if foreign.R == nil {
			foreign.R = &gameTitleR{}
		}
		foreign.R.GameGameMatches = append(foreign.R.GameGameMatches, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.GameID == foreign.ID {
				...
				foreign.R.GameGameMatches = append(foreign.R.GameGameMatches, local)
				break
			}
		}
	}
...
// loaded structs of the objects. This is for an N-1 relationship.
func (gameMatchL) LoadUser(ctx context.Context, e boil.ContextExecutor, singular bool, maybeGameMatch interface{}, mods queries.Applicator) error {
	var slice []*GameMatch
	var object *GameMatch
	...
	if singular {
		...
		foreign.R.GameMatches = append(foreign.R.GameMatches, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.UserID == foreign.ID {
				...
				foreign.R.GameMatches = append(foreign.R.GameMatches, local)
				break
			}
		}
	}
...
// SetGame of the gameMatch to the related item.
// Sets o.R.Game to related.
// Adds o to related.R.GameGameMatches.
func (o *GameMatch) SetGame(ctx context.Context, exec boil.ContextExecutor, insert bool, related *GameTitle) error {
	var err error
	if insert {
	...
	updateQuery := fmt.Sprintf(
		"UPDATE \"game_matches\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 1, []string{"game_id"}),
		strmangle.WhereClause("\"", "\"", 2, gameMatchPrimaryKeyColumns),
	)
	...
	if related.R == nil {
		related.R = &gameTitleR{
			GameGameMatches: GameMatchSlice{o},
		}
	} else {
		related.R.GameGameMatches = append(related.R.GameGameMatches, o)
	}

	return nil
}

// SetUser of the gameMatch to the related item.
// Sets o.R.User to related.
// Adds o to related.R.GameMatches.
func (o *GameMatch) SetUser(ctx context.Context, exec boil.ContextExecutor, insert bool, related *User) error {
	var err error
	if insert {
		if err = related.Insert(ctx, exec, boil.Infer()); err != nil {
			return errors.Wrap(err, "failed to insert into foreign table")
	...
	if related.R == nil {
		related.R = &userR{
			GameMatches: GameMatchSlice{o},
		}
	} else {
		related.R.GameMatches = append(related.R.GameMatches, o)
	}
...
func FindGameMatch(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*GameMatch, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "models_psql: unable to select from game_matches")
	}
...
func (o *GameMatch) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	if o == nil {
		return errors.New("models_psql: no game_matches provided for insertion")
	}
	...
	if err != nil {
		return errors.Wrap(err, "models_psql: unable to insert into game_matches")
	}
...
func (o *GameMatch) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		if len(wl) == 0 {
			return 0, errors.New("models_psql: unable to update game_matches, could not build whitelist")
		}
	...
	if err != nil {
		return 0, errors.Wrap(err, "models_psql: unable to update game_matches row")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models_psql: failed to get rows affected by update for game_matches")
	}
...
func (q gameMatchQuery) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "models_psql: unable to update all for game_matches")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models_psql: unable to retrieve rows affected for game_matches")
	}
...
func (o GameMatchSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "models_psql: unable to update all in gameMatch slice")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models_psql: unable to retrieve rows affected all in update all gameMatch")
	}
	return rowsAff, nil
}
...
func (o *GameMatch) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
		return errors.New("models_psql: no game_matches provided for upsert")
	}
	...
	if !cached {
		...
		if updateOnConflict && len(update) == 0 {
			return errors.New("models_psql: unable to upsert game_matches, could not build update column list")
		}
	...
	if err != nil {
		return errors.Wrap(err, "models_psql: unable to upsert game_matches")
	}
...
func (o *GameMatch) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	if o == nil {
		return 0, errors.New("models_psql: no GameMatch provided for delete")
	}
	...
	result, err := exec.ExecContext(ctx, sql, args...)
	if err != nil {
		return 0, errors.Wrap(err, "models_psql: unable to delete from game_matches")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models_psql: failed to get rows affected by delete for game_matches")
	}
...
func (q gameMatchQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	if q.Query == nil {
		return 0, errors.New("models_psql: no gameMatchQuery provided for delete all")
	}
	...
	result, err := q.Query.ExecContext(ctx, exec)
	if err != nil {
		return 0, errors.Wrap(err, "models_psql: unable to delete all from game_matches")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models_psql: failed to get rows affected by deleteall for game_matches")
	}
...
func (o GameMatchSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	result, err := exec.ExecContext(ctx, sql, args...)
	if err != nil {
		return 0, errors.Wrap(err, "models_psql: unable to delete all from gameMatch slice")
	}
...
func (o *GameMatchSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	if err != nil {
		return errors.Wrap(err, "models_psql: unable to reload all in GameMatchSlice")
	}
...
func GameMatchExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	...
	err := row.Scan(&exists)
	if err != nil {
		return false, errors.Wrap(err, "models_psql: unable to check if game_matches exists")
	}
